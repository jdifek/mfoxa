...
const TermsOfRegistrationComponent: React.FC<{ mfo: Mfo }> = ({ mfo }) => {
  ...
  const offers = mfo?.credit_offers || {};
                ...
                <SwiperSlide key={index}>
                  <div className="w-full rounded-lg bg-white p-[10px] md:p-[16px] shadow-md">
                    ...
                    {["amount", "term", "rate", "rrs"].map((key, i) => {
                      const value =
                        key === "amount"
                          ? offer?.amount?.formatted
                          : key === "term"
                          ? offer?.term?.formatted
                          : key === "rate"
                          ? offer?.rate?.formatted
                          : offer?.real_annual_rate?.formatted;
                    ...
                    <a
                      href={mfo?.apply_url || mfo?.get_money_button_url || "#"}
                      target="_blank"
